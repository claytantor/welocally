...
public class BusinessMetricsJob extends QuartzJobBean {
	
	static Logger logger = 
		Logger.getLogger(BusinessMetricsJob.class);
				...
				for (BusinessLocation bl : business.getLocations()) {
					...
					if(lastDateStore != null && 
						...
						for (BusinessMetrics businessMetrics : minedLocationMetrics) {
							...
							BusinessMetrics dayMetrics = 
								businessManagerService.findBusinessMetricsByLocationAndStartTime(
										bl, minedMidnight.getTime());
														
							//with the same start date and update it
							if(dayMetrics != null) {
								dayMetrics.setBoos(businessMetrics.getBoos());
								dayMetrics.setYays(businessMetrics.getYays());
								dayMetrics.setRatings(businessMetrics.getRatings());
								//update
								logger.debug("update");
								businessManagerService.saveBusinessMetrics(dayMetrics);
								
							} else {//otherwise we will save
								//save
