...
public class HibernateBusinessLocationDao 
	extends HibernateDaoSupport 
	implements BusinessLocationDao {

	static Logger logger = Logger.getLogger(HibernateBusinessLocationDao.class);
	...
	@Override
	public BusinessLocation findByPrimaryKey(final Long id) {
		return (BusinessLocation)getHibernateTemplateOverride().execute(new HibernateCallback() {
			public Object doInHibernate(Session session)
			throws HibernateException, SQLException 
			{

				Query query = session.createQuery(
					"select distinct entityimpl from "+BusinessLocation.class.getName()+
					" as entityimpl where entityimpl.id = :id");
				
				query.setLong("id", id);
				
				BusinessLocation t = (BusinessLocation)query.uniqueResult();
