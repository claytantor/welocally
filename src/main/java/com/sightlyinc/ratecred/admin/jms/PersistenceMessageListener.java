...
public class PersistenceMessageListener implements MessageListener { 
    ...
    public void onMessage(Message message) {
    	...
    	try {   
	    	if (message instanceof TextMessage) {
				...
				JSONObject jPersistenceActivity = new JSONObject(new String(tm.getText().getBytes()));
				
				//deserialize the json
				PersistenceActivity activity = 
            		jacksonMapper.readValue(
            				new ByteArrayInputStream(
            						tm.getText().getBytes()), PersistenceActivity.class);
				...
				if(clazz.isAnnotationPresent(PersistenceObservable.class)){
					if(clazz.getName().equals(Article.class.getName())){
						Article a = articleDao.findByPrimaryKey(activity.getEntityId());
					}
